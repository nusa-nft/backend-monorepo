...
import { ERC1155_dummy, ERC721_dummy } from "@nusa-nft/smart-contract/typechain-types"
...
import { TransferEvent } from "@nusa-nft/smart-contract/typechain-types/contracts/ERC721_dummy";

export async function importERC721Mint({
    restApi,
    db,
    web3Provider,
    erc721,
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // Mint some nfts
      let tx = await erc721.connect(minter).safeMint(
        receiver.address,
        metadataIpfsHash,
      );
      let receipt = await tx.wait();
      let transferSingleEvent = receipt.events?.find(e => e.event === 'Transfer') as TransferEvent;
      transferEvents.push(transferSingleEvent);
    }
    ...
    for (const [i, item] of items.entries()) {
      let [from, to, tokenId] = transferEvents[i].args;
      assert(item.tokenId.toNumber() === tokenId.toNumber(), fmtFailed(`Token ID mismatch, ${item.tokenId.toNumber()}, ${tokenId.toNumber()}`));
    ...
    for (const [i, tokenOwnership] of tokenOwnerships.entries()) {
      let [from, to, tokenId] = transferEvents[i].args;
      assert(tokenOwnership.tokenId.toNumber() === tokenId.toNumber(), fmtFailed(`Token ID mismatch, ${tokenOwnership.tokenId.toNumber()}, ${tokenId.toNumber()}`));
      ...
      assert(tokenOwnership.quantity === 1, fmtFailed(`Quantity mismatch, ${tokenOwnership.quantity}, ${1}`));
    }
    console.log(fmtSuccess('Imported NFTs Token ownership is correct'));

    // Mint some more nfts
    const metadataIpfsHashes_2 = [];
    const transferEvents_2: TransferEvent[] = [];
    for (let i = 5; i < 10; i++) {
      let resp = await uploadToFileToIpfs(`${__dirname}/../test-data/image1.png`);
      const fileIpfsHash = `ipfs://${resp.Hash}`;
      const metadata = {
      ...
      let receipt = await tx.wait();
      let transferSingleEvent = receipt.events?.find(e => e.event === 'Transfer') as TransferEvent;
      transferEvents_2.push(transferSingleEvent);
    ...
    await retry(async () => {
      itemsAfterImport = await db.item.findMany({
        where: {
          contract_address: erc721.address,
          tokenId: { in: transferEvents_2.map(ev => ev.args.tokenId.toString()) }
        },
        orderBy: { tokenId: 'asc' }
      });
    ...
    for (const [i, item] of itemsAfterImport.entries()) {
      let [from, to, tokenId] = transferEvents_2[i].args;
      assert(item.tokenId.toNumber() === tokenId.toNumber(), fmtFailed(`Token ID mismatch, ${item.tokenId.toNumber()}, ${tokenId.toNumber()}`));
